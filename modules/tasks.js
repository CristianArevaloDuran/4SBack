...
export async function doneTask(req, res) {
    ...
    try{
        ...
        const status = await statusModel.findOne({ code: "DONE" });
        if (!status) {
            return res.status(500).json({ message: "Server error" });
        }

        if (task.status.toString() === status._id.toString()) {
            const statusPending = await statusModel.findOne({ code: "PEN" });
            if (!statusPending) {
                return res.status(500).json({ message: "Server error" });
            }
            await task 
                .updateOne({ status: statusPending._id })
                .then(() => res.status(200).json({ message: "Task undone", task }))
                .catch((err) => res.status(500).json({ message: err.message }));
            return;
        }

        await task
            .updateOne({ status: status._id })
            .then(() => res.status(200).json({ message: "Task done", task }))
            .catch((err) => res.status(500).json({ message: err.message }));
    }
    catch(err){
        res.status(500).json({ message: err.message });
